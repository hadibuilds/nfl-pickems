...
export const calculateRankWithTies = (standings, username, selectedWeek = null) => {
  ...
  /**
   * Helper function to get point color (all white now)
   * @param {number} tier - Medal tier (not used anymore, kept for compatibility)
   * @returns {string} - Tailwind CSS class for text color
   */
  export const getPointColor = (tier) => {
    return 'text-white'; // White for all ranks
  };

  /**
   * Helper function to render rank (medal or number)
   * @param {number} medalTier - Medal tier (1=gold, 2=silver, 3=bronze, 4+=number)
   * @param {string|number} displayRank - Rank to display if not a medal
   * @returns {JSX.Element} - Medal component or rank number
   */
